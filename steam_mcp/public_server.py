...
def _catalog(status: dict[str, Any]) -> dict[str, Any]:
    value = {
        ...
        "capabilities": {
            ...
            "external_pricing": {"provider": "cheapshark", "opt_in": "pricing.options.include_external_deals", "currency": "USD", "max_stores": 5, "cache_seconds": 3600, "scope": "Tracked shops, not Steam regional history"},
            "market_analytics": {
                ...
                "note": "Configured key does not guarantee fields; check response access_mode and available_fields. Keyless Gamalytic is limited. Steam keys do not unlock Gamalytic plans.",
            },
        },
        "status": status,
    }
...
def _operation_schema(operation: str) -> dict[str, Any]:
    ...
    schemas: dict[str, dict[str, Any]] = {
        "steam_game_get": {"views": ["summary", "store", "compatibility", "technical", "dlc", "tags", "achievements", "live", "news", "pricing", "analytics"], "summary_store_select": ["appid", "name", "type", "is_free", "price", "initial_price", "discount_pct", "developers", "publishers", "release_date", "coming_soon", "genres", "categories", "features", "controller_support", "steam_deck", "platforms", "metacritic", "metacritic_url", "recommendations_total", "achievements_total", "dlc", "dlc_count", "required_age", "mature_content", "supported_languages", "full_audio_languages", "website", "short_description", "pc_requirements", "about_the_game"], "options": {"summary/store": ["include_requirements", "include_long_description"], "technical": ["section", "branch", "platform", "include_launch_options", "include_all_manifests"], "dlc": ["enrich", "on_sale_only"], "pricing": ["countries", "include_external_deals"], "analytics": {"providers": ["steam", "gamalytic", "steamspy"]}}, "technical_select": ["product", "branches", "depots", "current_build"], "achievements": "limit and cursor page items; select definitions and/or global_rates", "analytics": "official facts and estimates remain separate; provenance records fetched_at, available/missing fields and units; missing is not zero; unavailable sources preserve successes", "pricing": "include_external_deals=false by default; opt in for up to five CheapShark stores and tracked-store all-time low in USD; no regional availability guarantee"},
